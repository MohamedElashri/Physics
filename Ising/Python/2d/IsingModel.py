...
from tqdm import tqdm # fancy progress bars for loops
...
T = np.arange(1.6,3.25,0.01) # Initlaize temperature range (the range includes critical temperature) > takes form np.arange(start,stop,step)
...
'''
Magnetization calculation: Calculate the Magnetization of a given configuration
...
def mc(s,Temp,n):
    for m in range(n):
        ...
        elif random.random() < np.exp(-ediff/Temp): # if not accept it with probability exp^{-dE/kT}
            s[i,j] = -s[i,j]
    return s


'''
Calculate the physical quantities of interest 
1- Energy 
2- Mangtization 
3- Heat Capacity
'''
...
plt.plot(T, En_avg, marker='.', color='IndianRed')
plt.xlabel("Temperature (T)", fontsize=20);
plt.ylabel("Energy ", fontsize=20);         plt.axis('tight');

sp =  f.add_subplot(2, 2, 2 );
plt.plot(T, abs(mag), marker='.', color='RoyalBlue')
plt.xlabel("Temperature (T)", fontsize=20); 
plt.ylabel("Magnetization ", fontsize=20);   plt.axis('tight');

sp =  f.add_subplot(2, 2, 3 );
plt.plot(T, CV, marker='.', color='IndianRed')
plt.xlabel("Temperature (T)", fontsize=20);  
